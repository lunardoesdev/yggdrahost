...
export async function weatherBotApp(config: Config) {
    ...
    bot.command(["start", "help"], async (ctx: Context) => {
        return ctx.reply("use /weather location command, for example: /weather London")
    })

    await bot.api.setMyCommands([
        { command: "start", description: "Start the bot" },
        { command: "help", description: "Show help text" },
        { command: "weather", description: "/weather London will check weather in London for example" },
    ]);
